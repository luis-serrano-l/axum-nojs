...
`wo-caps` is server-side feature detection. The page carries a few empty elements; CSS
...
<div class="wo-caps" aria-hidden="true">
  <i class="wo-cap wo-cap-probed"></i>
  <i class="wo-cap wo-cap-invokers"></i>
  …one per flag…
</div>
```
...
```css
.wo-cap-probed { background-image: url("/wo/caps?flag=probed"); }
@supports selector(:popover-open) {
  .wo-cap-popover { background-image: url("/wo/caps?flag=popover"); }
}
```
...
`GET /wo/caps?flag=<name>` answers `204 No Content` with `Cache-Control: no-store` and

```
Set-Cookie: wo-cap-<name>=1; Path=/; Max-Age=2592000; SameSite=Lax
```
...
`webonsive/examples/hyper_server.rs` wires it by hand in six lines.

## 3. The cookie format

One cookie per flag, `wo-cap-<name>=1`, never a list. The beacons load in parallel: seven
responses each writing `wo-caps=<old list + me>` would overwrite one another and keep one flag.
...
Without `wo-cap-probed=1` it returns `Caps::ASSUMED` instead, whatever else it finds.
...
5. `cargo test -p wo-caps`: the round-trip and one-rule-per-flag tests cover the new flag
