...
async fn index_under_another_palette() {
    ...
    assert!(page.exists("style.nojs-tokens"), "layout_with emits the token overrides");
    ...
    for path in demo::PATHS {
        ...
        for (suffix, cookie) in [("modern", MODERN), ("fallback", OLD)] {
            ...
            if !inside_shadow_root {
                ...
                assert!(page.is_visible(".nojs-header"), "{path} ({suffix}) has no header");
            }
...
async fn dialog_variants_follow_caps() {
    ...
    assert!(page.is_visible(".nojs-dialog-danger form[method=post][action='/dialog/delete'] button.nojs-danger"), "confirm is a real form");
    assert!(page.exists("form input[type=hidden][name=returns_to][value='/dialog']"));
    assert!(page.is_visible("a.nojs-dialog-cancel[href='#']") && page.is_visible("a.nojs-dialog-close[href='#']"), "fallback closes through links");
    ...
    assert!(page.exists("dialog > .nojs-dialog-close:last-child"), "close control is last so focus lands on the field");
}

#[tokio::test]
async fn tabs_strip_versus_accordion() {
    let modern = Page::render(demo::router(), "/tabs?tab.demo=1", MODERN).await;
    assert_eq!(modern.display(".nojs-tabs details").as_deref(), Some("contents"));
    let first = modern.bbox(".nojs-tabs summary").unwrap();
    let second = modern.bbox(".nojs-tabs details:nth-of-type(2) summary").unwrap();
    assert!((first.y - second.y).abs() < 1.0, "tab titles share a row: {first:?} {second:?}");
    assert!(second.x > first.x + first.width - 1.0);
    let panel = modern.bbox(".nojs-tabs details[open] .nojs-tabs-panel").unwrap();
    assert!(panel.y >= first.y + first.height - 2.0, "open panel sits below the strip: {panel:?} vs {first:?}");
    assert!(!modern.is_visible(".nojs-tabs details:nth-of-type(1) .nojs-tabs-panel"), "closed panel hidden");
    assert_eq!(modern.text("#nojs-tabs-demo details[open] .nojs-tabs-badge").as_deref(), Some("3"));
    assert!(modern.exists("#nojs-tabs-demo details:nth-of-type(3) .nojs-tabs-lazy"), "lazy tab has no body until opened");
    ...
    let mark = modern.bbox("#nojs-tabs-demo details[open] .nojs-tabs-mark").unwrap();
    let open_title = modern.bbox("#nojs-tabs-demo details[open] summary").unwrap();
    ...
    assert!(lazy.is_visible("#nojs-tabs-demo details:nth-of-type(3) .nojs-tabs-panel p"), "lazy tab rendered once open");
    let side_first = modern.bbox("#nojs-tabs-side summary").unwrap();
    let side_second = modern.bbox("#nojs-tabs-side details:nth-of-type(2) summary").unwrap();
    let side_panel = modern.bbox("#nojs-tabs-side details[open] .nojs-tabs-panel").unwrap();
    ...
    assert_eq!(old.display(".nojs-tabs details").as_deref(), Some("block/flow"));
    let first = old.bbox(".nojs-tabs summary").unwrap();
    let second = old.bbox(".nojs-tabs details:nth-of-type(2) summary").unwrap();
...
async fn accordion_multi_open_with_controls_and_nesting() {
    ...
    assert_eq!(page.count("#nojs-accordion-faq > details[open]"), 2, "two sections open at once");
    assert!(page.exists("#nojs-accordion-faq > details:not([name])"), "multi group has no name attribute");
    assert!(page.exists("#nojs-accordion-faq-more > details[name='faq-more'][open]"), "nested group is exclusive and open");
    assert!(page.is_visible("#nojs-accordion-faq-more"), "nested accordion sits inside the open third section");
    assert_eq!(page.text(".nojs-accordion-controls a:first-child").as_deref(), Some("Expand all"));
    assert!(page.exists(".nojs-accordion-controls a[href='/accordion?open.faq=0%2C1%2C2&open.faq-more=0']"), "expand all links to every index and keeps the nested key");
    assert!(page.exists(".nojs-accordion-controls a[href='/accordion?open.faq=&open.faq-more=0']"), "collapse all sets the key to nothing, explicitly, so the cookie cannot reopen them");
    ...
    assert!(page.is_visible("#nojs-accordion-faq > details:nth-of-type(2) .nojs-accordion-summary"), "summary line shows on a closed section");
    assert!(!page.is_visible("#nojs-accordion-faq > details:nth-of-type(1) > summary .nojs-accordion-summary"), "summary line hidden once open");
    assert!(page.is_visible("#nojs-accordion-faq > details:nth-of-type(1) .nojs-accordion-icon"));
    let outer = page.bbox("#nojs-accordion-faq > details:nth-of-type(3) > summary").unwrap();
    let inner = page.bbox("#nojs-accordion-faq-more > details:nth-of-type(1) > summary").unwrap();
...
async fn combobox_chips_results_and_create_row() {
    ...
    assert_eq!(page.count(".nojs-combobox-chip"), 1, "one chip for the selection");
    assert!(page.exists(".nojs-combobox-chip input[type=hidden][name=sel][value=Zig]"), "the chip rides along with the next search");
    assert!(page.exists(".nojs-combobox-chip a[href='/combobox?q=ru'][aria-label='Remove Zig']"), "the chip's link removes it");
    ...
    assert_eq!(page.text(".nojs-combobox-status").as_deref(), Some("2 matches"));
    ...
    assert_eq!(none.text(".nojs-combobox-status").as_deref(), Some("No matches."));
    assert!(none.exists("form.nojs-combobox-create[action='/combobox/new'] input[name=name][value=elixir]"), "create row posts the text");
    assert!(none.exists("form.nojs-combobox-create input[name=sel][value=Zig]"), "and keeps the selection");
...
async fn popover_variants() {
    ...
    assert!(modern.exists("#account .nojs-popover-heading") && modern.count("#account .nojs-popover-sep") == 2);
    assert!(modern.exists("#account a[aria-disabled=true]:not([href])"), "disabled link has no href");
    assert!(modern.exists("#account form[method=post][action='/popover/signout'] button.nojs-popover-danger"), "action is a post form");
    ...
    assert!(modern.exists(".nojs-popover-end nav#more[popover]"), "placement class");
    let old = Page::render(demo::router(), "/popover", OLD).await;
    assert!(old.is_visible(".nojs-popover-details summary"));
    assert!(!old.is_visible(".nojs-popover-details nav"), "closed details hides the menu");
...
async fn settings_flash_and_form_values() {
    ...
    let (ok, warn, danger) = (page.bbox(".nojs-flash-ok").unwrap(), page.bbox(".nojs-flash-warn").unwrap(), page.bbox(".nojs-flash-danger").unwrap());
...
async fn caps_table_lists_every_flag() {
    ...
    assert_eq!(fresh.count(".nojs-cap"), 9, "unknown browser gets one beacon per flag");
}
...
async fn blitz_has_no_declarative_shadow_dom() {
    ...
    let boxes: Vec<_> = (1..=3).map(|i| fallback.bbox(&format!(".nojs-stream-section:nth-of-type({i})")).unwrap()).collect();
...
async fn table_sort_links_and_pages() {
    ...
    assert_eq!(page.count(".nojs-table thead th a"), 2, "every visible column header is a sort link");
    ...
    assert!(page.exists(".nojs-table-cols a[aria-pressed=false][href*='cols=name%2Csize%2Ckind']"), "the chooser links to showing Kind again");
    assert!(page.exists(".nojs-table-cols a[aria-pressed=true][href*='cols=size']"), "and to hiding Name");
    ...
    assert_eq!(page.count("tbody input[type=checkbox][name=row][form='nojs-table-files-bulk']"), 5, "a checkbox per row, owned by the bulk form");
    assert!(page.exists("form#nojs-table-files-bulk[method=post][action='/table/bulk'] button[name=action][value=archive]"));
    assert_eq!(page.count("tbody .nojs-table-menu .nojs-popover"), 5, "a menu per row");
    assert_eq!(page.count("tbody details.nojs-table-detail"), 5, "a detail block per row");
    assert!(!page.is_visible("tbody details.nojs-table-detail .nojs-table-detail-body"), "detail closed by default");
    ...
    assert_eq!(page.text(".nojs-paged-table-range").as_deref(), Some("6–10 of 21"));
    assert!(page.exists("select[name='per.files'] option[value='5'][selected]"));
    assert!(page.exists(".nojs-paged-table-per input[name=cols][value='name,size']"), "the page-size form keeps the columns");
    assert!(page.exists("a.nojs-paged-table-end[href*='page=1']") && page.exists("a.nojs-paged-table-end[href*='page=5']"), "first and last links");
    assert!(page.exists(".nojs-paged-table-jump input[type=number][name=page][max='5'][value='2']"), "jump-to-page form");
    assert!(page.exists(".nojs-paged-table-jump input[type=hidden][name='per.files'][value='5']"), "the jump keeps the page size");
    assert!(page.is_visible(".nojs-paged-table-jump button"));

    // 36 files at 5 a page is 8 pages: page 5 numbers 1 … 4 5 6 7 8.
    let long = Page::render(demo::router(), "/table?page=5", "nojs-cap-probed=1; nojs-ui=per.files=5").await;
    assert_eq!(long.text(".nojs-paged-table-range").as_deref(), Some("21–25 of 36"), "page size from the nojs-ui cookie");
    assert_eq!(long.count(".nojs-paged-table-gap"), 1, "one ellipsis before the current page's neighbours");
    assert!(long.exists("a.nojs-paged-table-end[href='/table?per.files=5&page=8']"), "Last names the remembered size");
    ...
    assert_eq!(empty.text(".nojs-table-empty").as_deref(), Some("No files match this filter."));
    let loading = Page::render(demo::router(), "/table?loading=1", MODERN).await;
    assert!(loading.exists("tbody[aria-busy=true]") && loading.count("tr.nojs-table-skeleton") == 3, "loading body is marked busy and drawn as skeleton rows");
}
...
async fn swap_targets_render_without_script() {
    ...
    assert!(!page.exists("form [data-nojs=swap]"), "the form is not inside a root");
    assert!(page.exists("a[data-nojs-push='false'][data-nojs-target='#count']"), "the quiet link is a plain link");
    assert!(page.exists("form[data-nojs-indicator='#saving']") && !page.is_visible("#saving"), "indicator hidden without the script");
    assert!(!page.exists("[data-nojs-busy], [aria-busy]"), "nothing is busy without the script");
}
...
async fn counter_and_inputs() {
    ...
    assert!(page.exists(".nojs-color-presets button[aria-pressed=true][value='#b3261e']"), "the saved colour is the pressed preset");
    assert_eq!(page.text(".nojs-color code").as_deref(), Some("#b3261e99"), "opacity 60% in the code");
    ...
    assert!(page.is_visible(".nojs-select-search input[name=country-q]") && page.exists(".nojs-select-search button[formmethod=get][formaction='/inputs']"), "21 options: a filter box");
    assert!(!page.exists(".nojs-select-search input[name=size-q]"), "3 options: none");
    assert!(page.exists("#size option[value=l] .nojs-select-icon"), "icons in options");
...
async fn form_groups_help_counters_and_layouts() {
    ...
    assert_eq!(page.count("form.nojs-form fieldset.nojs-form-group legend"), 2, "two groups with legends");
    ...
    assert_eq!(page.text("output.nojs-field-count[for=f-bio]").as_deref(), Some("0 / 160"));
...
async fn wizard_marks_steps() {
    ...
    assert!(page.exists(".nojs-wizard-done a[href='/wizard?step.signup=0']"), "done step links back");
    assert!(!page.exists(".nojs-wizard-steps li:nth-child(3) a"), "future step is not a link");
    assert!(page.is_visible("input[type=hidden][name=step][value='1'] ~ fieldset") || page.is_visible(".nojs-wizard-form fieldset"));
    assert!(page.exists("a.nojs-wizard-back[href='/wizard?step.signup=0']"));
    let steps: Vec<_> = (1..=3).map(|i| page.bbox(&format!(".nojs-wizard-steps li:nth-child({i})")).unwrap()).collect();
    ...
    assert!(!first.exists(".nojs-wizard-back"), "no Back on the first step");
    assert!(first.is_visible("progress.nojs-wizard-progress[value='0'][max='2']"), "progress bar");
    assert!(!first.exists(".nojs-wizard-resume"), "nothing to resume");
    assert!(page.exists(".nojs-wizard-steps li:nth-child(2) small"), "the optional step says so");
    ...
    let back = Page::render(demo::router(), "/wizard", &format!("{MODERN}; nojs-ui=step.signup=2; wizard=name=Ada&email=ada@example.org&digest=weekly")).await;
    assert_eq!(back.text("li[aria-current=step]").as_deref(), Some("Review"));
    assert!(back.is_visible(".nojs-wizard-resume a[href='/wizard?step.signup=0']"), "resume notice with Start over");
    assert_eq!(back.count(".nojs-wizard-review dd a.nojs-wizard-edit"), 4, "an Edit link per value");
    assert!(back.exists("a.nojs-wizard-edit[aria-label='Edit Topics'][href='/wizard?step.signup=1']"));
    assert_eq!(back.text(".nojs-wizard-review dd:nth-of-type(4) .nojs-note").as_deref(), Some("(skipped)"));
}
...
async fn toasts_stack_in_the_corner() {
    ...
    assert!(page.exists(".nojs-toast-danger[role=alert]") && page.exists(".nojs-toast-ok[role=status]"));
...
async fn drawer_is_a_sidebar_when_wide_and_breadcrumbs_fold() {
    ...
    assert!(page.is_visible(".nojs-drawer-panel nav a[aria-current=page]"), "wide viewport: the closed dialog shows as a sidebar");
    assert!(!page.is_visible(".nojs-drawer-open"), "wide viewport: no menu button");
    let (side, content) = (page.bbox(".nojs-drawer-panel").unwrap(), page.bbox(".nojs-drawer-content").unwrap());
    ...
    assert!(page.exists(".nojs-breadcrumbs-fold details"), "the long trail folds");
    assert!(!page.is_visible(".nojs-breadcrumbs-fold ol"), "folded middle is closed");
    let old = Page::render(demo::router(), "/nav", OLD).await;
    assert!(old.exists("a.nojs-drawer-open[href='#site']"), "fallback opener is a :target link");
}
...
async fn stats_and_empty_state() {
    ...
    let (a, b) = (page.bbox(".nojs-stat-grid > :nth-child(1)").unwrap(), page.bbox(".nojs-stat-grid > :nth-child(2)").unwrap());
    assert!((a.y - b.y).abs() < 1.0 && b.x > a.x, "cards sit side by side when there is room");
    assert!(page.exists(".nojs-stat-good") && page.exists(".nojs-stat-bad"), "down_is_good flips the colour");
    assert!(page.exists("a.nojs-stat[href='/table']"), "a card can be a link");
...
async fn command_palette_suggests_and_lists_matches() {
    ...
    assert!(page.is_visible(".nojs-palette-results a[href='/table']"), "results for a partial query");
