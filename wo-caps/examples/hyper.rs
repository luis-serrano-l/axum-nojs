//! wo-caps on raw hyper, nothing else: `cargo run -p wo-caps --example hyper`, then open
...
use wo_caps::{BEACON_PATH, Cap, Caps, beacon_cookie, beacon_css, beacons};
...
async fn handle(req: Request<Incoming>) -> Result<Response<Full<Bytes>>, Infallible> {
    ...
    let reply = match (req.method(), req.uri().path()) {
        ...
        (&Method::GET, "/") => {
            ...
            let page = format!(
                "<!DOCTYPE html><meta charset=utf-8><title>wo-caps</title><style>{}</style>\
