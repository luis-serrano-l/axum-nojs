...
{% endif %}    ui: Ui,
    ...
    let Some(p) = params(&mut form) else {
        ...
        let title = "New {{ snake_singular | replace(from="_", to=" ") }}";
        let body = views::{{ snake_plural }}::form(&ui, title, "/{{ snake_plural }}", form.values(), &errors.pairs());
...
{% endif %}    ui: Ui,
    ...
    let Some(p) = params(&mut form) else {
        let errors = form.errors();
        let title = "Edit {{ snake_singular | replace(from="_", to=" ") }}";
        let body = views::{{ snake_plural }}::form(&ui, title, &action, form.values(), &errors.pairs());
        return Ok(ui.page(title, body).into_response());
    };
