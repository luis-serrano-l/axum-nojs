...
fn params(form: &mut Submitted) -> Option<Params> {
    ...
    let body = form.optional::<String>("body");
...
async fn create(
    ...
    let Some(p) = params(&mut form) else {
        ...
        let title = "New note";
        let body = views::notes::form(&ui, title, "/notes", form.values(), &errors.pairs());
...
async fn update(
    ...
    let Some(p) = params(&mut form) else {
        let errors = form.errors();
        let title = "Edit note";
        let body = views::notes::form(&ui, title, &action, form.values(), &errors.pairs());
        return Ok(ui.page(title, body).into_response());
    };
