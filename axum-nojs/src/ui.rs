...
impl Ui {
    ...
    pub fn params<'a>(&'a self, key: &'a str) -> impl Iterator<Item = &'a str> + 'a {
        self.params.iter().filter(move |(k, _)| k == key).map(|(_, v)| v.as_str())
    }

    /// A whole page titled `title` in this request's theme.
    pub fn page(&self, title: &str, body: Markup) -> Page {
        Page { caps: self.caps, theme: self.theme, title: title.to_string(), tokens: None, body, cookies: self.state.set_cookies() }
    }
    ...
    pub fn redirect(&self, to: &str) -> Redirect {
        Redirect { to: to.to_string(), messages: Vec::new(), cookies: Vec::new() }
    }
...
impl Render for Page {
    fn render(&self) -> Markup {
        match &self.tokens {
            Some(t) => layout::layout_with(&self.caps, &self.title, self.theme, t, self.body.clone()),
...
impl Redirect {
    ...
    pub fn set_cookies(&self) -> Vec<String> {
        let flash = (!self.messages.is_empty()).then(|| {
            let pairs: Vec<(Level, &str)> = self.messages.iter().map(|(l, m)| (*l, m.as_str())).collect();
            // A lone info message stays plain text.
            let text = match pairs.as_slice() { [(Level::Info, m)] => m.to_string(), _ => stack(&pairs) };
            format!("{FLASH_COOKIE}={}; Path=/; Max-Age=60; SameSite=Lax", encode(&text))
        });
        flash.into_iter().chain(self.cookies.iter().cloned()).collect()
    }
    ...
    pub fn into_http<B: From<String>>(self) -> http::Response<B> {
        let mut res = http::Response::builder().status(303).header(http::header::LOCATION, &self.to);
...
mod axum_glue {
    ...
    fn with_cookies(mut res: Response, cookies: impl IntoIterator<Item = String>) -> Response {
        for c in cookies {
            res.headers_mut().append(header::SET_COOKIE, HeaderValue::from_str(&c).expect("cookie is ASCII"));
        }
        res
    }
...
mod tests {
    ...
    fn one_value_carries_caps_theme_state_and_flash() {
        let ui = Ui::from_request("/t", "tab.t=1&caps=popover", "theme=light; nojs-ui=open.f=2");
        ...
        assert_eq!((q.param("q"), q.params("sel").collect::<Vec<_>>()), (Some("a b"), vec!["x", "y,z"]));
    ...
    fn a_page_carries_the_state_it_changed() {
        ...
        assert_eq!(page.set_cookies().len(), 2, "tab.t remembered, the flash cleared");
        let html = page.tokens(&Tokens::default()).into_string();
        assert!(html.contains(r#"data-theme="dark""#) && html.contains("<title>T</title>") && html.contains("nojs-tokens"));
    }
    ...
    fn redirect_messages_stack_in_call_order() {
        ...
        assert_eq!(plain.set_cookies(), ["nojs-flash=Saved.; Path=/; Max-Age=60; SameSite=Lax"]);
        let two = ui.redirect("/s").ok("Saved.").warn("Look.").cookie("x=1".into());
        let cookies = two.set_cookies();
        assert!(cookies[0].starts_with("nojs-flash=ok%3ASaved.%0Awarn%3ALook.") && cookies[1] == "x=1");
