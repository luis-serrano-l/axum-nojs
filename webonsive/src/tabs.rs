...
//! - `view-transition-name` (Chrome 111, Firefox 144, Safari 18) on the open summary: the
//!   strip's highlight slides to the new tab, across documents through the layout's
//!   `@view-transition` rule and in place with the enhancement script.
//!
        ...
        div id=[state.map(|_| format!("wo-tabs-{name}"))] data-wo=[state.map(|_| "swap")]
            ...
            @for (i, t) in tabs.iter().enumerate() {
                details name=(name) open[i == active] {
                    summary style=[(i == active).then(|| format!("view-transition-name: wo-tabs-{name}"))] {
                        @match state {
...
/* Vertical: titles in the first column, the open panel spans every row of the second. */
.wo-tabs.wo-tabs-vertical { display: grid; grid-template-columns: max-content 1fr; border-bottom: 0; }
.wo-tabs.wo-tabs-vertical summary {
  grid-column: 1; border-bottom: 0; border-right: 2px solid transparent; margin: 0 -1px 0 0;
}
.wo-tabs.wo-tabs-vertical details[open] summary { border-right-color: var(--wo-accent); }
.wo-tabs.wo-tabs-vertical details::details-content { grid-column: 2; grid-row: 1 / span var(--wo-tabs-n, 1); }
...
@media (max-width: 40rem) {
  .wo-tabs:not(.wo-accordion):has(.wo-tabs-select) summary { display: none; }
